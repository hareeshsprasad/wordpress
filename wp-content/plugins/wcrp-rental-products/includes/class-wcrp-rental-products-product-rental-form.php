						...
						<script>
							jQuery(document).ready(function($) {
								...
								function rentalFormAdvancedPricingAjaxRequest(advancedPricingTotal, advancedPricingCartItemPrice) {
									...
									var rentalFormAdvancedPricingAjaxRequestData = {
										...
										'data': {
											...
											<?php } ?> 'rent_from': $('#wcrp-rental-products-rent-from-<?php echo esc_html($this->rental_form_id); ?>').val(),
											...
											?> '_GET_<?php echo esc_html($get_key); ?>': '<?php echo esc_html($get_value); ?>',
													<?php // Adds _GET_ data, this is each $_GET, useful for advanced pricing calculations based off ?rent_period=x, etc 
													?>
											<?php
												}
											}
									...
									rentalFormAdvancedPricingAjaxRequest.done(function(rentalFormAdvancedPricingAjaxRequestResponse) {
										...
										if (rentalFormAdvancedPricingAjaxRequestResponse !== 'error') {
											...
											$('#wcrp-rental-products-rental-totals-<?php echo esc_html($this->rental_form_id); ?>').attr('style', 'display: block !important;');
											<?php // Not .show() as !important required (see related public CSS for info) 
											?>
								...
								function rentalFormReset() {
									...
									$('#wcrp-rental-products-available-rental-stock-totals-<?php echo esc_html($this->rental_form_id); ?>').attr('style', 'display: none !important;');
									<?php // Not .hide() as !important required (see related public CSS for info) 
									?>
									$('#wcrp-rental-products-rental-totals-<?php echo esc_html($this->rental_form_id); ?>').attr('style', 'display: none !important;');
									<?php // Not .hide() as !important required (see related public CSS for info) 
									?>
														...
														if (isNaN(rentalPrice)) {
															...
															if (true == lockDaysCheckPassed) {
																...
																if (!isNaN(rentalPrice) && '' !== rentFromDate && '' !== rentToDate) {
																	...
																	pricingTierPercent = 0;
																	<?php // If there aren't any matching days greater than the rental days then % is 0 (no change) 
																	?>
																	...
																	?>
																		...
																		for (var i = 0; i < pricingTiersData.days.length; i++) {
																			...
																			if (parseInt(pricingTiersData.days[i]) > pricingTierHighest) {
																				<?php // parseInt as days 
																				?>

																				if (rentedDays > parseInt(pricingTiersData.days[i])) {

																					pricingTierHighest = parseInt(pricingTiersData.days[i]);
																					<?php // parseInt as days 
																					?>
																					pricingTierPercent = parseFloat(pricingTiersData.percent[i]);
																					<?php // parseFloat as can be multiple decimal places 
																					?>
																	...
																	if (totalOverrides.hasOwnProperty(rentedDays)) {
																		<?php // Note that this will be empty if period_selection, see $total_overrides_json conditions 
																		?>
																	...
																	?>
																		...
																		if (rentalFormUpdateAjaxRequestResponseJSON.rent_from !== '' && rentalFormUpdateAjaxRequestResponseJSON.rent_to !== '') {
																			<?php // Stops available on dates message appearing before dates set 
																			?>
																			...
																			if (rentalFormAvailableRentalStockTotalsShow == true) {
																				...
																				$('#wcrp-rental-products-available-rental-stock-totals-<?php echo esc_html($this->rental_form_id); ?>').attr('style', 'display: block !important;');
																				<?php // Not .show() as !important required (see related public CSS for info), this is not hidden at the start of this code block as it gets hidden upon rental form reset/update then this runs 
																				?>
									...
									if (addRentalProductsPopup == true) {
										...
										var rentalFormAddToOrderAjaxRequestData = {
											...
											<?php if ('variable' == $product_type) { ?> 'product_id': $('#wcrp-rental-products-rental-form-wrap-<?php echo esc_html($this->rental_form_id); ?>').find('.cart input[name="product_id"]').val(),
												'variation_id': $('#wcrp-rental-products-rental-form-wrap-<?php echo esc_html($this->rental_form_id); ?>').find('.cart input[name="variation_id"]').val(),
											<?php } else { ?> 'product_id': $('#wcrp-rental-products-rental-form-wrap-<?php echo esc_html($this->rental_form_id); ?>').find('.cart button[name="add-to-cart"]').val(),
											<?php } ?> 'cart_item_price': $('#wcrp-rental-products-cart-item-price-<?php echo esc_html($this->rental_form_id); ?>').val(),
											'rent_from': $('#wcrp-rental-products-rent-from-<?php echo esc_html($this->rental_form_id); ?>').val(),
											'rent_to': $('#wcrp-rental-products-rent-to-<?php echo esc_html($this->rental_form_id); ?>').val(),
											'return_days_threshold': $('#wcrp-rental-products-return-days-threshold-<?php echo esc_html($this->rental_form_id); ?>').val(),
											<?php if (true == $in_person_pick_up_return_allowed) { ?> 'in_person_pick_up_return': $('#wcrp-rental-products-in-person-pick-up-return-<?php echo esc_html($this->rental_form_id); ?>').val(),
								...
								?>
										...
										let rentalPrice = NaN;
										<?php // No price set, will show as unavailable, see other NaN conditions why, this may be initially set as NaN for variable products as the parent product doesn't have the _wcrp_rental_products_rental_purchase_price meta, but rentalPrice gets updated upon variation selection to the variation price if populated (or returns NaN) 
										?>
								...
								<?php } else { ?>
									...
									<?php // This is already via wc_get_price_to_display() so inc/exc tax, doesn't matter that this is converted to a float, as if price is empty the rental form (inc this code) doesn't get used (no price set) 
									?>
												...
												*/
												...
												?>
													...
													if (rentalProductVariations[k]['wcrp_rental_products_rental_purchase_price'] !== 'error') {
														...
														rentalPrice = NaN;
														<?php // See other NaN conditions why 
														?>
								...
								var rentalFormCalendar = new Litepicker({
									...
									parentEl: '#wcrp-rental-products-rental-dates-parent-<?php echo esc_html($this->rental_form_id); ?>',
									<?php // Fixes issue with total days tooltip not positioned correctly with inlineMode enabled 
									?>
									...
									setup: (rentalFormCalendar) => {
										...
										rentalFormCalendar.on('render', (ui) => {
											...
											?>
												...
												if (rentalFormAvailabilityCheckerPopulationInitial == true) {
													<?php // Ensures only runs once not on every render 
													?>
											...
											?>
												...
												if (disableRentalStartEndDays.includes(date.getDay())) {
													...
													?>
														...
														<?php // If start or end (not start/end) then dotted border as per notice, pointer-events not set to none as needs to be clickable for the start or end date depending on which is allowed 
														?>
										...
										rentalFormCalendar.on('preselect', (date1, date2) => {
											...
											if (doAutoSelectEndDate == true) {
												...
												var newDate2 = date1.add(<?php echo esc_html($minimum_days); ?> - 1, 'days');
												<?php // -1 ensures end date correct 
												?>
										...
										rentalFormCalendar.on('selected', (date1, date2) => {
											...
											?>
												...
												if (date1.getTime() == date2.getTime()) {
													<?php // Not .getDate() as this is just the day number, so if used that would cause 13th June to 13th July to be matched as one day 
													?>
											...
											?>
												...
												?>
													...
													if (disableRentalStartEndDays.includes(date2.getDay())) {
														...
														alert('<?php esc_html_e('Sorry, these dates cannot be selected as they end on the highlighted days.', 'wcrp-rental-products'); ?>');
														<?php // The highlighting is a border, but we don't say this here, as potentially it might have been changed with CSS, in that scenario it would be expected the related text setting would also be changed so no longer specifies the border, however because there isn't a text setting for this particular alert we just leave it as highlighted 
														?>
														rentalFormCalendar.clearSelection();
										...
										rentalFormCalendar.on('clear:selection', () => {
											...
											rentalFormReset();
											<?php // Form is reset as add to cart button may have been enabled, without this resetting it's possible to add to cart using the previously selected dates even though the rental dates field is empty 
											?>
